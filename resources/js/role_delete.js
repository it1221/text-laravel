$(function () {
    let delete_buttons = $('[id^="delete_role_"]');
    ...
    delete_buttons.on('click', function () {
        let $this = $(this);
        let role = $this.attr('id').substring(12); //delete_role_IDのIDだけ切り出す

        let user = $this.data('user-id');
        let add_button = $(`#add_role_${role}`);
        ...
        $.ajax( {
            ...
            url: `/roles/${user}/detach`, //リクエストURL '/roles/{user}/detach'
            method: 'PUT', //形式
            dataType: 'json',
            data: { 
                'user' : user,
            ...
            .done(function(res)
            {
                ...
                add_button.toggleClass('disabled');
